...
def start_theme_interpretation(request, theme_id):
    ...
    # For Level 1 themes without subthemes, we create a pseudo-subtheme context
    # or adapt the service to work directly with themes
    try:
        # Check if theme has subthemes
        subthemes = theme.subthemes.all()
        
        if subthemes.exists():
            # If theme has subthemes, redirect to first one
            ...
            # For Level 1 themes, create a default subtheme automatically
