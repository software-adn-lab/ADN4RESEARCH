...
class ThemeDiscoveryService:
    ...
    def propose_code_normalization(
        ...
        codes_data = [
            {'code': code.code, 'frequency': code.frequency}
            for code in initial_codes
        ]
        
        # Call LLM to analyze and propose normalizations
        proposals_data = self.llm_client.propose_code_normalization(codes_data)
        ...
        logger.info(f"Created {len(proposals)} normalization proposals")
        return proposals
    ...
    def accept_normalization_proposals(
        ...
        for proposal in proposals:
            ...
            total_frequency = sum(
                InitialCode.objects.filter(
                    code=orig_code,
                    project=proposal.project
                ).values_list('frequency', flat=True).first() or 0
        ...
        logger.info(f"Accepted {len(normalized_codes)} normalization proposals")
        return normalized_codes

    @transaction.atomic
    def propose_theme_structure(
        self, project=None
    ) -> List[ThemeDiscoveryProposal]:
        """
        ...
        logger.info(f"Created {len(proposals)} theme proposals")
        return proposals
