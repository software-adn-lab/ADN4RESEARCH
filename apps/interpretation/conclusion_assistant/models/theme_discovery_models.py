...
class ThemeDiscoveryProposal(models.Model):
    ...
    proposed_subthemes = models.JSONField(
        default=list
    )  # Lista de subtemas con sus códigos
    rationale = models.TextField(blank=True)  # Explicación de la IA
    status = models.CharField(
        max_length=20, choices=ProposalStatus.choices, default=ProposalStatus.PENDING
    )
    codes_used = models.ManyToManyField(
        "interpretation.NormalizedCode", related_name="theme_proposals"
    )
